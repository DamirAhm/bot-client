...
import {
	DraggableEntity,
	DroppableEntity,
} from "../../../../Common/DragAndDropEntities";
...
const ScheduleSection: React.FC<{}> = ({}) => {
	...
	useSubscription<{
		onScheduleChanged: { dayIndex: number; newSchedule: string[] };
	...
	useEffect(() => {
		...
		if (scheduleQuery.data?.schedule) {
			...
			newScheduleData.initialSchedule = Array.from(
...
const ScheduleDay: React.FC<ScheduleDayProps> = memo(
	({ index, lessons, lessonsList, changeDay, initialLessons }) => {
		...
		const addLesson = useCallback(() => {
			setChanges(
				changes.concat([changes[changes.length - 1] || lessonsList.current[0]])
			);
		}, [changes, lessonsList]);

		const newLessonsList = useRef([
			...new Set([...lessonsList.current, ...changes]),
		]);
		useEffect(() => {
			newLessonsList.current = [
				...new Set([...lessonsList.current, ...changes]),
			];
		}, [changes]);
		...
		const onDragEnd: OnDragEndResponder = initial => {
			...
			if (destination && source) {
				let newDay = changes.filter((_, i) => i !== source.index);

				let lesson = changes[source.index];
				newDay.splice(destination.index, 0, lesson);
...
const Lesson: React.FC<LessonProps> = memo(
	({
		...
		const lessonChangeHandler = (
			...
			if (
				["create-option", "select-option"].includes(meta.action) &&
				isOptionType(value)
			) {
		...
		const lessonOptions = lessonsList.current.map(les => ({
			value: les,
			label: les,
		}));
		...
		return (
			...
			>
				{!changing ? (
					...
					<div
						className={styles.lessonChange}
						key={`picker${dayIndex + lesson + index}`}
					>
						<CreatableSelect
