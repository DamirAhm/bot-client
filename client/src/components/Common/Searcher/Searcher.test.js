...
test("input has given value", () => {
    let searcher = render(<Searcher text="test text" />);

    expect(searcher.queryByTestId("input").value).toBe("test text");

    searcher.unmount();

    searcher = render(<Searcher text="different text" />);
...
test("calls changeHandler on change of input", () => {
    ...
    const searcher = render(<Searcher changeHandler={changeHandler} />);

    const input = searcher.queryByTestId("input");

    fireEvent.change(input, { target: { value: "awdaw" } });
...
test("clear button send empty value to change handler", () => {
    ...
    const searcher = render(
        <Searcher changeHandler={changeHandler} text="123" />
    );
