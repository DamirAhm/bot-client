...
test("renders list of passed attachments", () => {
    ...
    expect(images.some(img => img.getAttribute("src") === "1 src")).toBe(true)
    expect(images.some(img => img.getAttribute("src") === "2 src")).toBe(true)
});
...
test("stab opens photo modal", () => {
    ...
    const Stab = ({onClick}: {onClick: () => void}) => <div onClick={onClick}>stab</div>;
...
test("connect images adds prevImg and nextImg to images", () => {
    ...
    expect(connectedImages.every((conImg,i) => conImg.src === images[i].url)).toBe(true);
...
test("passes all props except for url", () => {
    ...
    expect(Object.keys(connectedImages[0]).sort()).toEqual(["_id", "alt", "nextImg", "prevImg", "src"]);
    expect(Object.keys(connectedImages[1]).sort()).toEqual(["_id", "nextImg", "prevImg", "src", "style"]);
})